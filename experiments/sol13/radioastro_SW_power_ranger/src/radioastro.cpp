...
bool InitDevice(CFFT & fft, std::complex<TFXP>* &sample_buffer, std::complex<TFXP>* &coeff, int nfft, int chunk_size, bool log)
{
    if (log) {
        printf("\n\nThis program requires that the bitstream is loaded in the FPGA.\n");
        printf("Press ENTER to confirm that the bitstream is loaded (proceeding without it can crash the board).\n\n");
    ...
    sample_buffer = (std::complex<TFXP>*)fft.AllocDMACompatible(chunk_size * nfft * sizeof(std::complex<TFXP>), 1);
    coeff = (std::complex<TFXP>*)fft.AllocDMACompatible(chunk_size * nfft * sizeof(std::complex<TFXP>), 1);
...
int main(int argc, char* argv[]) {
    ...
    std::complex<TFXP>* batch_in = nullptr;
    std::complex<TFXP>* batch_out = nullptr;
    ...
    welch_psd(signal, Nseg, sample_rate, (int)signal_param.fc, freqs, config.log2_nfft, psd, times, &powerRanger, energies, &fft_accel, batch_in, batch_out, chunk_size);
