...
void load_nfft_value(Cplx* in, stream_cplx &out, ap_uint<32> numFFT);
...
void retrieve_nfft_value(stream_cplx &in, Cplx* out, ap_uint<32> numFFT);

//Top function :
void Top_DataFlox(Cplx* in, Cplx* out, ap_uint<32> num);
...
void fft_HW(Cplx* input, Cplx* output, ap_uint<32> numFFT){
...
#pragma HLS INTERFACE m_axi port=output offset=slave bundle=gmem0 depth=1024 \
                        max_write_burst_length=64 num_write_outstanding=8
...
//pragma HLS PIPELINE
	Cplx BufferIN[nFFT];
    ap_uint<32> reversed;
    load_counter :for (ap_uint<32> counter = 0 ; counter < numFFT ; counter ++){
	#pragma HLS LOOP_TRIPCOUNT avg= 5
    	for (ap_uint<32> y = 0 ; y < nFFT ; y ++){
		#pragma HLS PIPELINE II=1
    		BufferIN[y] = in[y+ counter*nFFT];
    	}
		load_total :for (ap_uint<32> i = 0; i < nFFT; i++) {
