...
bool CompareVectors(float* sw_output, ap_uint<32>* hw_output, ap_uint<32> size){
	bool success = true;
	float* hw2compare = new float[nFFT];

	std::memcpy(hw2compare,hw_output,size*sizeof(float));

	for (ap_uint<32> i = 0 ; i < size ; i ++) {
		float error = sw_output[i] - hw2compare[i];
		if (error*error > 0.001){
			success = false ;
			printf("@iter %u:\n", (unsigned)i);
			printf("Error : abs error : %f\n",error*error);
			printf("HW : %f, SW : %f\n",hw2compare[i],sw_output[i]);
		}
	}
	return success;
}
...
int main(int argc, char ** argv)
{
	...
	ap_uint<32>* output_HW = new ap_uint<32>[nFFT];
	//ap_uint<64>*  batch_in = new ap_uint<64>[nFFT*chunk_size];
	...
	for (int i = 0 ; i < num-3 ; i ++) {
		...
		fft_SW(buffer,log2nFFT,output_SW);
		for (int k = 0 ; k < nFFT ; k ++){
			spectrum[k] += (output_SW[k].real()*output_SW[k].real() + output_SW[k].imag()*output_SW[k].imag())/factor;
		}
	}
	...
	Processing_HW(input_HW,output_HW,8);

	Processing_HW(input_HW+5*nFFT,output_HW,num-5);




	printf("post HW : \n");
	if(CompareVectors(spectrum,output_HW,nFFT)){
