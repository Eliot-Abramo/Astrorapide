...
void load_initial_buffer(Cplx* Buff0,Cplx* Buff1,Cplx* Buff2,ap_uint<64>* input,float* Spect,ap_uint<32>* output);
...
void normaling(stream_cplx &in, float* out,ap_uint<32> numFFT);



//Top function :
void Top_DataFlox(axi_cplx* in, float* out, ap_uint<32> num, Cplx* Buff0,Cplx* Buff1,Cplx* Buff2);
...
#pragma HLS INTERFACE s_axilite port=return
	...
	Cplx Buff1[nFFT];
	Cplx Buff2[nFFT];
	float Spectrumoutput[nFFT];
	//ap_uint<32> BurstBRAM[nFFT];


	load_initial_buffer(Buff0,Buff1,Buff2,input,Spectrumoutput,output);
	Top_DataFlox(input, Spectrumoutput, numFFT,Buff0, Buff1, Buff2);
	std::memcpy(output,Spectrumoutput,nFFT*sizeof(float));
}
...
void load_initial_buffer(Cplx* Buff0,Cplx* Buff1,Cplx* Buff2,ap_uint<64>* input,float* Spect,ap_uint<32>* output){
	...
	std::memcpy(Spect,output,nFFT*sizeof(float));
}
...
#pragma HLS INLINE off
	int factor = 19456 ;
	for (ap_uint<32> counter = 0 ; counter < numFFT-3 ; counter ++) {
		for (ap_uint<32> jj = 0 ; jj < nFFT ; jj ++){
		#pragma HLS PIPELINE II = 1
			Cplx temp = in.read() ;
			out[jj]+= (temp.real()*temp.real()+temp.imag()*temp.imag())/factor ;
		}
	}
}
