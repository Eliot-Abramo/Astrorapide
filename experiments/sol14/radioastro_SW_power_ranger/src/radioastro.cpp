...
bool InitDevice(CXADCProxy & xadc, bool log=false)
{
    ...
    if ( xadc.Open(XADC_HW_ADDR, MAP_SIZE) != CAccelProxy::OK ) {
        printf("Error mapping adc device at physical address 0x%08X\n", XADC_HW_ADDR);
        return false;
    }
    if (log)
        printf("Device at physical adc address 0x%08X successfully mapped into the application virtual address space\n\n",
            XADC_HW_ADDR);

    return true;
}

bool InitDevice(CFFT & fft, std::complex<float>* &sample_buffer, float* &psd_buff, int nfft, int chunk_size, bool log)
{
    if (log) {
        printf("\n\nThis program requires that the bitstream is loaded in the FPGA.\n");
        printf("Press ENTER to confirm that the bitstream is loaded (proceeding without it can crash the board).\n\n");
    ...
    if (fft.Open(DRIVER_NAME) != CAccelProxy::OK ) {
    ...
    psd_buff = (float*)fft.AllocDMACompatible(nfft * sizeof(float), 1);

    // std::memcpy(psd_buff, 0, nfft * sizeof(float));
    
    if (log)
    {
        printf("DMA memory allocated.\n");
        printf("Input: Virtual address: 0x%08X (%u)\n", (uint32_t)sample_buffer , (uint32_t)sample_buffer );
        printf("Output: Virtual address: 0x%08X (%u)\n", (uint32_t)psd_buff , (uint32_t)psd_buff);

    }

    if ((sample_buffer == NULL) || (psd_buff  == NULL))
    {
        if (log)
...
int main(int argc, char* argv[]) {
    ...
    if (!InitDevice(powerRanger, false))
      return -1;
    ...
    float *psd_buff = nullptr;
    const int chunk_size = 512; 
    if (!InitDevice(fft_accel, batch_in, psd_buff, nfft, chunk_size, false))
        return -1;
    ...
    welch_psd(signal, Nseg, sample_rate, (int)signal_param.fc, freqs, config.log2_nfft, psd_buff, times, &powerRanger, energies, &fft_accel, batch_in, chunk_size);
    ...
    energy_start_segment = powerRanger.GetEnergy();
    clock_gettime(CLOCK_MONOTONIC_RAW, &start_segment);
    moving_average(psd_buff, nfft, config.ma_w, psd_ma);
    ...
    free(peak_height);
    free(velocities);
    fft_accel.FreeDMACompatible(psd_buff);
