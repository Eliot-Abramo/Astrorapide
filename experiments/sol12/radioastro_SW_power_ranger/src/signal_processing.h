...
void welch_psd(std::complex<float>* samples, int Nseg, float Fs, float Fc, float* freqs, int log2_nfft, float* spectrum_buff, TTimes & times, CXADCProxy * powerRanger, TEnergies & energies, CFFT * fft_accel, std::complex<float> * batch_in, int chunk_size);
