...
bool InitDevice(CFFT & fft, std::complex<float>* &sample_buffer, std::complex<float>* &coeff, int nfft, int chunk_size, bool log)
{
    ...
    sample_buffer = (std::complex<float>*)fft.AllocDMACompatible(chunk_size * nfft * sizeof(std::complex<float>), 0);
    coeff = (std::complex<float>*)fft.AllocDMACompatible(chunk_size * nfft * sizeof(std::complex<float>), 0);
