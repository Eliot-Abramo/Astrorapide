...
//bool InitDevice(CFFT & fft, std::complex<float>* &sample_buffer, std::complex<float>* &coeff, int nfft, bool log);
void welch_psd(std::complex<float>* samples, int Nseg, float Fs, float Fc, float* freqs, int log2_nfft, float* spectrum, TTimes & times, CXADCProxy * powerRanger, TEnergies & energies, CFFT * fft_accel, std::complex<float> * batch_in, std::complex<float> * batch_out, int chunk_size);
