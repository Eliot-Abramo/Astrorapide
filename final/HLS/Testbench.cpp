...
int main(int argc, char ** argv)
{
	...
	std::complex<float>* output_SW = new std::complex<float>[(num) * nFFT];
	...
	ap_uint<64>* output_HW = new ap_uint<64>[nFFT*(num)];
	...
	for (int i = 0 ; i < num ; i ++){
		fft_SW(input_SW+i*nFFT,log2nFFT,output_SW+i*nFFT);
	}
	...
	printf("post HW : \n");
	if(CompareVectors(output_SW,output_HW,nFFT*(num))){
