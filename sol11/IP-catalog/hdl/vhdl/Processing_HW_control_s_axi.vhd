...
entity Processing_HW_control_s_axi is
generic (
...
end entity Processing_HW_control_s_axi;
...
-- 0x24 : reserved
-- 0x28 : Data signal of numFFT
--        bit 9~0 - numFFT[9:0] (Read/Write)
--        others  - reserved
-- 0x2c : reserved
-- (SC = Self Clear, COR = Clear on Read, TOW = Toggle on Write, COH = Clear on Handshake)

architecture behave of Processing_HW_control_s_axi is
    ...
    constant ADDR_NUMFFT_DATA_0   : INTEGER := 16#28#;
    constant ADDR_NUMFFT_CTRL     : INTEGER := 16#2c#;
    ...
    signal int_numFFT          : UNSIGNED(9 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_NUMFFT_DATA_0 =>
                        rdata_data <= RESIZE(int_numFFT(9 downto 0), 32);
                    when others =>
                        NULL;
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    numFFT               <= STD_LOGIC_VECTOR(int_numFFT);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_NUMFFT_DATA_0) then
                    int_numFFT(9 downto 0) <= (UNSIGNED(WDATA(9 downto 0)) and wmask(9 downto 0)) or ((not wmask(9 downto 0)) and int_numFFT(9 downto 0));
                end if;
            end if;
        end if;
    end process;
